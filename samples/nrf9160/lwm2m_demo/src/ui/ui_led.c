...
#endif
	default:
		LOG_ERR("Error %d: PWM channel %u not supported", -ENOTSUP, led_num);
		return -ENOTSUP;
	}
...
int ui_led_pwm_on_off(uint8_t led_num, bool new_state)
{
	...
	ret = pwm_pin_set_usec(led_dev, pwm_channel, PWM_PERIOD_USEC, pulse_width[led_num] * new_state,
				LED_PWM_FLAGS);
	if (ret) {
		LOG_ERR("Error %d: set LED PWM pin %u failed", ret, led_num);
		return ret;
	}
...
static int get_gpio_pin(uint8_t led_num, gpio_pin_t *gpio_pin)
{
	...
	/* LEDs are named led_1 .. led_3 in Thingy devicetree, but led_0 .. led_4 in DK devicetree */
	...
	case 3:
		*gpio_pin = LED_GPIO_PIN(3);
		break;
	default:
		LOG_ERR("Error %d: LED GPIO pin %u not supported", -ENOTSUP, led_id);
		return -ENOTSUP;
	}
...
int ui_led_gpio_init(void)
{
	...
	for (int i = 0; i < NUM_LEDS; ++i) {
		...
		if (ret) {
			LOG_ERR("Error %d: configure LED pin %d failed", ret, i);
			return ret;
		}
	}
