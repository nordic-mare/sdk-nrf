...
#define LOG_MODULE_NAME ipso_obj_buzzer
...
#define BUZZER_FREQUENCY_ID	7001
...
static struct lwm2m_engine_obj_field fields[] = {
	OBJ_FIELD(BUZZER_ON_OFF_ID, RW, BOOL),
	OBJ_FIELD(BUZZER_LEVEL_ID, RW, FLOAT64),
	OBJ_FIELD(BUZZER_DELAY_DURATION_ID, RW_OPT, FLOAT64),
	OBJ_FIELD(BUZZER_MINIMUM_OFF_TIME_ID, RW, FLOAT64),
	OBJ_FIELD(BUZZER_FREQUENCY_ID, RW, FLOAT32),
	OBJ_FIELD(BUZZER_APPLICATION_TYPE_ID, RW_OPT, STRING),
	OBJ_FIELD(BUZZER_DIGITAL_STATE_ID, R, BOOL),
};
...
static struct lwm2m_engine_obj_inst *buzzer_create(uint16_t obj_inst_id)
{
	...
	buzzer_data[avail].level.val1 = 80; /* 80% */
	buzzer_data[avail].delay_duration.val1 = 1; /* 1 seconds */
	buzzer_data[avail].frequency.val1 = 1000;
	...
	INIT_OBJ_RES(BUZZER_ON_OFF_ID, res[avail], i, res_inst[avail], j, 1, false,
	...
	INIT_OBJ_RES_DATA(BUZZER_FREQUENCY_ID, res[avail], i, res_inst[avail], j,
			  &buzzer_data[avail].frequency,
			  sizeof(buzzer_data[avail].frequency));
...
int ipso_buzzer_init(const struct device *dev)
{
