...
static void set_timestamp(uint16_t obj_inst_id)
{
	...
	snprintk(path, MAX_LWM2M_PATH_LEN, "%d/%u/%d", IPSO_OBJECT_COLOUR_ID, obj_inst_id,
		 TIMESTAMP_RID);
...
int lwm2m_init_light_sensor(void)
{
	...
	lwm2m_engine_set_res_data(LWM2M_PATH(IPSO_OBJECT_COLOUR_ID, LIGHT_OBJ_INSTANCE_ID,
				  ...
				  &meas_qual_ind_light_sensor, sizeof(meas_qual_ind_light_sensor),
	...
	lwm2m_engine_set_res_data(LWM2M_PATH(IPSO_OBJECT_COLOUR_ID, COLOUR_OBJ_INSTANCE_ID,
					     MEASUREMENT_QUALITY_INDICATOR_RID),
				  &meas_qual_ind_colour_sensor, sizeof(meas_qual_ind_colour_sensor),
				  LWM2M_RES_DATA_FLAG_RW);
#endif
