...
static uint32_t intensity_to_duty_cycle_divisor(void)
{
	return MIN(
...
{
	if ((new_frequency < BUZZER_MIN_FREQUENCY) ||
		(new_frequency > BUZZER_MAX_FREQUENCY)) {
		return -EINVAL;
	}
...
{
	if ((new_intensity < BUZZER_MIN_INTENSITY) ||
	    (new_intensity > BUZZER_MAX_INTENSITY)) {
		return -EINVAL;
	}
