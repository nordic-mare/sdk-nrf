...
* The ``<info>`` value is an integer providing additional information about the event.
  ...
  * *Positive value* - Number of bytes received through the App Data Container object or the Binary
...
* The ``<data>`` parameter is a string that contains the data received through the App Data Container object
  or the Binary App Data container object.
...
Specifying ``instance_id`` and ``resource_instance_id`` in the ``app_data`` command sends the data
to the Binary App Data Container object instead of the App Data Container object.
