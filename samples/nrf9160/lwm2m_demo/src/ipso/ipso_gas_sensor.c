...
#define LOG_MODULE_NAME ipso_gas_sensor
...
#define IPSO_OBJECT_ID 3325

#define SENSOR_NAME "Air quality"

#define CURRENT_CALIBRATION_RID 5821

#define MAX_INSTANCE_COUNT CONFIG_LWM2M_IPSO_GAS_SENSOR_INSTANCE_COUNT

#define NUMBER_OF_OBJ_FIELDS 9

// NUMBER_OF_OBJ_FIELDS - number of EXEC fields
#define RESOURCE_INSTANCE_COUNT (NUMBER_OF_OBJ_FIELDS - 1)

#define UNIT_STR_MAX_SIZE 8 // TODO: Change to actual size
#define APPLICATION_STR_MAX_SIZE 17
...
static float32_value_t current_calibration[MAX_INSTANCE_COUNT];
static char application_type[MAX_INSTANCE_COUNT][APPLICATION_STR_MAX_SIZE];
...
static struct lwm2m_engine_obj_field fields[] = {
    ...
    OBJ_FIELD_DATA(CURRENT_CALIBRATION_RID, RW_OPT, FLOAT32),
...
gas_sensor_create(uint16_t obj_inst_id)
{
	int index, i = 0, j = 0;
    ...
    current_calibration[index].val1 = 0;
    current_calibration[index].val2 = 0;
	...
	INIT_OBJ_RES_DATA(SENSOR_UNITS_RID, res[index], i, res_inst[index], j,
			  sensor_units[index], UNIT_STR_MAX_SIZE);
    ...
    INIT_OBJ_RES_DATA(CURRENT_CALIBRATION_RID, res[index], i, 
              res_inst[index], j, &current_calibration[index],
              sizeof(*current_calibration));
    INIT_OBJ_RES_DATA(APPLICATION_TYPE_RID, res[index], i, res_inst[index], 
              j, &application_type[index], sizeof(*application_type));
...
static int ipso_gas_sensor_init(const struct device *dev)
{
	...
	sensor.create_cb = gas_sensor_create;
	lwm2m_register_obj(&sensor);

	return 0;
}

SYS_INIT(ipso_gas_sensor_init, APPLICATION,
