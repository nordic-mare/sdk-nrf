...
static void set_timestamp(uint16_t obj_inst_id)
{
	...
	snprintk(path, MAX_LWM2M_PATH_LEN, "%d/%u/%d", IPSO_OBJECT_PUSH_BUTTON_ID, obj_inst_id,
		 TIMESTAMP_RID);
...
#if defined(CONFIG_LWM2M_IPSO_PUSH_BUTTON_VERSION_1_1)
	lwm2m_engine_set_res_data(LWM2M_PATH(IPSO_OBJECT_PUSH_BUTTON_ID, BUTTON1_OBJ_INST_ID,
					     TIMESTAMP_RID),
...
#if defined(CONFIG_LWM2M_IPSO_PUSH_BUTTON_VERSION_1_1)
	lwm2m_engine_set_res_data(LWM2M_PATH(IPSO_OBJECT_PUSH_BUTTON_ID, BUTTON2_OBJ_INST_ID,
					     TIMESTAMP_RID),
