...
#define SENSE_LED_GPIO_NAME  DT_GPIO_LABEL(GPIO_SENSE_RED_NODE_ID, gpios)
...
{
    int ret;
    ret = gpio_pin_set(sense_led_gpio_dev, RED_SENSE_LED_GPIO_PIN, new_state);
    if (ret) {
        LOG_ERR("Could not set red sense LED. (%d)", ret);
        return ret;
    }
    ret = gpio_pin_set(sense_led_gpio_dev, GREEN_SENSE_LED_GPIO_PIN, new_state);
    if (ret) {
        LOG_ERR("Could not set green sense LED. (%d)", ret);
        return ret;
    }
    ret = gpio_pin_set(sense_led_gpio_dev, BLUE_SENSE_LED_GPIO_PIN, new_state);
    if (ret) {
        LOG_ERR("Could not set blue sense LED. (%d)", ret);
        return ret;
    }
    return 0;
}
...
int ui_sense_led_init(void)
{
	...
	ret = gpio_pin_configure(sense_led_gpio_dev, RED_SENSE_LED_GPIO_PIN, 
                    SENSE_LED_GPIO_FLAGS | GPIO_OUTPUT_INACTIVE);
    if (ret) {
        LOG_ERR("Could not configure red sense LED. (%d)", ret);
		return ret;
	}
    ret = gpio_pin_configure(sense_led_gpio_dev, GREEN_SENSE_LED_GPIO_PIN, 
                    SENSE_LED_GPIO_FLAGS | GPIO_OUTPUT_INACTIVE);
    if (ret) {
        LOG_ERR("Could not configure green sense LED. (%d)", ret);
		return ret;
	}
    ret = gpio_pin_configure(sense_led_gpio_dev, BLUE_SENSE_LED_GPIO_PIN, 
                    SENSE_LED_GPIO_FLAGS | GPIO_OUTPUT_INACTIVE);
	if (ret) {
        LOG_ERR("Could not configure blue sense LED. (%d)", ret);
		return ret;
	}
